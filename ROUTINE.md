...
| Frame hero (frame 1, panel dưới ảnh thật) bị cắt mất dòng cuối của subline ở đáy khung 1080x1920 — chỉ phát hiện được khi người dùng xem lại video/thumbnail đã đăng, KHÔNG bị `npm run check` bắt (Layout checker báo `0 issues` cả trước lẫn sau khi sửa) | Khối nội dung trong panel dưới (masthead absolute offset + `.f1-pills{margin-top:9cqh}` + `.f1-textblock{margin-top:3.6cqh}` + headline 9.2cqw thường xuống 3 dòng + subline 5.4cqw/line-height 1.4 xuống 2 dòng, cộng dồn margin/gap) cao hơn chiều cao khả dụng của panel (45%×1920 trừ padding đáy 5cqh) — phần tràn bị `.f1-scene{overflow:hidden}` cắt mất hoàn toàn (không phải chữ nhỏ dần hay tràn ra ngoài nhìn thấy được, mà biến mất). `npm run check`'s Layout lint không phát hiện được lỗi tràn-do-overflow-hidden kiểu này — đừng coi "Layout: 0 issues" là bằng chứng đủ, GATE D 8.5d (soát hình ảnh bằng mắt) mới là lưới an toàn thật sự cho lỗi này | Khi dựng frame có panel chia đôi ảnh/chữ (kiểu frame 1), LUÔN tự cộng nhẩm chiều cao khối nội dung (mọi margin-top + gap + số dòng dự kiến × line-height thực tế của headline/subline) so với chiều cao khả dụng của panel (%×1920 trừ padding đáy) TRƯỚC khi render — nếu khối gần chạm hoặc vượt đáy, giảm bớt margin-top/gap, giảm cỡ chữ, hoặc tăng tỷ lệ panel (vd 55/45 → 48/52) ngay từ đầu. Ở Bước 8.5d, khi soát ảnh 3 mốc + mốc giữa mỗi frame, đặc biệt chú ý cạnh ĐÁY của khung (không chỉ tìm chữ chồng lên nhau) — chữ bị cắt mất ở mép dưới dễ bị bỏ sót hơn chữ chồng đè vì không "trông sai" rõ ràng, chỉ thiếu 1 dòng. Nếu video đã lỡ đăng lên 4 kênh trước khi phát hiện lỗi này, sửa + render lại + cập nhật file trong repo vẫn nên làm (để lần đăng lại/video sau đúng), nhưng bài đã đăng public thường KHÔNG cập nhật lại được qua API (YouTube/Facebook/Instagram/Threads không hỗ trợ thay video/thumbnail sau khi đăng qua các action `publish_*` hiện có) — ghi rõ vào COMPLIANCE.md là bài cũ vẫn mang lỗi |
